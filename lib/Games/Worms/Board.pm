...
sub run {
  ...
  @Worm_names = ('Games::Worms::Random', 'Games::Worms::Random',
...
sub init_grid {
  ...
  for(my $row = 0; $row < $it->{'cells_high'}; ++$row) {
    ...
    for(my $col = 0; $col < $it->{'cells_wide'}; ++$col) {
      ...
      my $row_type_top = ((1 + $row) % 2);  # 1, 0, 1, 0, 1, 0, ...
      if($row_type_top) {  # Rows 0, 2, 4...
        my $sw = $here->toward('node', 4)->toward('node', 0);
        $here->{'nodes_toward'}[5] = $sw;
          $sw->{'nodes_toward'}[2] = $here;
      } else {  # Rows 1, 3, 5...
        my $se = $here->toward('node', 5)->toward('node', 3);
  ...
  for(my $row = 0; $row < $it->{'cells_high'}; ++$row) {
    ...
    for(my $col = 0; $col < $it->{'cells_wide'}; ++$col) {
      ...
      if($row_type_top) { # rows 0,2,4,...
        ...
        #           \ / \ / \ /            \ /      s2 s3
        #                                  n_d4
        my $n_d3 = $n->toward('node',3);
        my $n_d4 = $n->toward('node',4);

        $s1 = $Seg->new_at($x_base, $y_base, $x_base + $Tri_base,
                                $y_base, 'board', $it);
        # @{$s1->{'nodes'}} = ($n, $n_d3);
        $n->{'segments_toward'}[3] = $n_d3->{'segments_toward'}[0] = $s1;

        $s2 = $Seg->new_at($x_base, $y_base, $x_base + $Tri_base / 2,
                                $y_base + $Tri_height, 'board', $it);
        # @{$s2->{'nodes'}} = ($n, $n_d4);
        $n->{'segments_toward'}[4] = $n_d4->{'segments_toward'}[1] = $s2;

        $s3 = $Seg->new_at($x_base + $Tri_base / 2, $y_base + $Tri_height,
                                $x_base + $Tri_base, $y_base, 'board', $it);
        # @{$s3->{'nodes'}} = ($n_d3, $n_d4);
        ...
        my $n_d3 = $n->toward('node',3);
        my $n_d4 = $n->toward('node',4);
        my $n_d5 = $n->toward('node',5);

        $s1 = $Seg->new_at($x_base + $Tri_base / 2, $y_base, $x_base +
                                $Tri_base * 1.5, $y_base, 'board', $it);
        # @{$s1->{'nodes'}} = ($n, $n_d3);
        $n->{'segments_toward'}[3] = $n_d3->{'segments_toward'}[0] = $s1;

        $s2 = $Seg->new_at($x_base + $Tri_base / 2, $y_base, $x_base,
                                $y_base + $Tri_height, 'board', $it);
        # @{$s2->{'nodes'}} = ($n, $n_d5);
        $n->{'segments_toward'}[5] = $n_d5->{'segments_toward'}[2] = $s2;

        $s3 = $Seg->new_at($x_base + $Tri_base, $y_base + $Tri_height,
                                $x_base + $Tri_base / 2, $y_base, 'board', $it);
        # @{$s3->{'nodes'}} = ($n, $n_d4);
...
sub refresh_and_draw_grid {
  my $board = $_[0];
  $board->init_grid unless $board->{'segments'};
  foreach my $seg ( @{$board->{'segments'}} ) {
    $seg->refresh_and_draw;
  }
  return;
}
